...
function MessageTimeline({ currentUserId, messages }: { currentUserId: number; messages: CommunicationMessage[] }) {
  return <div className="space-y-3">{messages.map((message, index) => { const outgoing = message.direcao === "SAIDA"; const simulated = outgoing && message.simulada; const mine = message.autor?.id === currentUserId; const previous = messages[index - 1]; const showDate = !previous || new Date(previous.createdAt).toDateString() !== new Date(message.createdAt).toDateString(); return <div key={message.id}>{showDate && <div className="my-3 flex items-center gap-3 text-xs text-[var(--text-muted)]"><span className="h-px flex-1 bg-[var(--border-default)]" /><span>{formatCommunicationDate(message.createdAt, false)}</span><span className="h-px flex-1 bg-[var(--border-default)]" /></div>}<article className={`flex ${outgoing ? "justify-end" : "justify-start"}`}><div className={`max-w-[76%] rounded-lg border px-3 py-2 ${outgoing ? mine ? "border-emerald-200 bg-emerald-50" : "border-sky-200 bg-sky-50" : "border-[var(--border-default)] bg-[var(--bg-surface)]"}`}><p className="whitespace-pre-wrap break-words text-xs leading-5 text-[var(--text-primary)]">{message.texto || "Mensagem sem conteúdo"}</p><p className="mt-1 text-xs text-[var(--text-muted)]">{simulated ? `Simulação registrada por ${message.autor?.nome ?? "Automação"} às ${formatCommunicationTime(message.createdAt)} · Não enviada` : outgoing ? `Respondido por ${message.autor?.nome ?? "Automação"} às ${formatCommunicationTime(message.createdAt)}` : `Recebida às ${formatCommunicationTime(message.createdAt)}`}{outgoing && !simulated && message.statusEntrega ? ` · ${deliveryLabel(message.statusEntrega)}` : ""}</p></div></article></div>; })}</div>;
}
