import { AlertTriangle, Archive, Camera, CheckCircle2, Clock3, Globe2, History, Inbox, MessageCircle, PanelRightOpen, RefreshCw, Search, Send, StickyNote, UserPlus, UsersRound } from "lucide-react";
...
export default function DashboardInboxPanel({ authSession, initialConversationId }: InboxPanelProps) {
  ...
  const listQuery = useMemo(() => ({
    ...
    ...(queueView === "sla-atencao" ? { sla: "ATENCAO" as const } : {}),
    ...(queueView === "sla-critico" ? { sla: "CRITICO" as const } : {}),
  ...
  useEffect(() => {
    ...
    async function loadInitialDetail() {
      ...
      try {
        ...
        setLeaseOwned(detail.reservaResposta?.usuarioId === currentUserId);
        if (detail.naoLidas > 0) {
          void markCommunicationConversationRead(conversationId).then(() => {
            setConversation((value) => value?.id === conversationId ? { ...value, naoLidas: 0 } : value);
            void loadList(true);
          }).catch(() => undefined);
        }
      } catch (error) {
  ...
  }, [currentUserId, loadDetail, loadList, selectedId]);
  ...
  const canReplyDirectly = conversation?.podeResponderDiretamente !== false;
      ...
      <Surface className="inbox-workspace grid min-h-[520px] overflow-hidden">
        <aside className="inbox-filters flex min-h-0 flex-col border-r border-[var(--border-default)] bg-[var(--bg-muted)]" aria-label="Filas e filtros">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-2">
            ...
            <QueueButton active={queueView === "minhas"} icon={<MessageCircle size={14} />} label="Minhas conversas" onClick={() => { setQueueView("minhas"); setPage(1); }} />
            <QueueButton active={queueView === "sem-responsavel"} icon={<UserPlus size={14} />} label="Não atribuídas" onClick={() => { setQueueView("sem-responsavel"); setPage(1); }} />
            ...
            <p className="mb-1 mt-4 px-2 text-[10px] font-semibold uppercase text-[var(--text-muted)]">Prioridade</p>
            <QueueButton active={queueView === "sla-atencao"} icon={<Clock3 size={14} />} label="SLA em atenção" onClick={() => { setQueueView("sla-atencao"); setPage(1); }} />
            <QueueButton active={queueView === "sla-critico"} icon={<AlertTriangle size={14} />} label="SLA crítico" onClick={() => { setQueueView("sla-critico"); setPage(1); }} />
          </div>
        ...
        <section className="inbox-conversation-list flex min-h-0 flex-col border-r border-[var(--border-default)]" aria-label="Lista de conversas">
          <div className="flex items-center gap-2 border-b border-[var(--border-default)] p-3"><div className="relative min-w-0 flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--icon-muted)]" size={13} /><input aria-label="Buscar conversas" className="h-9 w-full rounded-md border border-[var(--control-border)] bg-[var(--control-bg)] pl-8 pr-3 text-xs outline-none focus:border-[var(--control-border-focus)] focus:ring-2 focus:ring-[var(--control-ring)]" onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Contato ou interesse" value={search} /></div><IconButton aria-label="Atualizar conversas" disabled={listRefreshing} onClick={() => void loadList()}><RefreshCw className={listRefreshing ? "animate-spin" : ""} size={14} /></IconButton></div>
        ...
        <section className="inbox-conversation flex min-h-0 min-w-0 flex-col bg-[var(--bg-surface)]" aria-label="Conversa selecionada">
          {!selectedId ? <EmptyState className="m-auto max-w-sm" description="Escolha uma conversa para consultar o histórico e responder." icon={<MessageCircle size={18} />} title="Selecione uma conversa" /> : detailLoading ? <LoadingState className="p-4" rows={6} /> : detailError ? <ErrorState className="m-4" description={detailError} onRetry={() => selectedId && void loadDetail(selectedId)} title="Falha ao abrir a conversa" /> : conversation && <>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2"><h2 className="truncate text-sm font-semibold text-[var(--text-primary)]">{conversation.contatoCanal.cliente?.nome ?? conversation.contatoCanal.nome ?? "Contato sem nome"}</h2><ChannelBadge conversation={conversation} /><ConversationStatusBadge status={conversation.status} /><ConversationSlaBadge sla={conversation.sla} /></div>
                    <p className="mt-0.5 truncate text-[11px] text-[var(--text-muted)]">Responsável: {conversation.responsavelPrincipal?.nome ?? "Fila compartilhada"} · Última atividade: {formatCommunicationTime(conversation.ultimaMensagemEm)}</p>
                  </div>
                </div>
                <div className="flex flex-wrap items-center justify-end gap-1">
                  ...
                  {conversation.status === "EM_ATENDIMENTO" && canChangeConversation && <Button disabled={busy} onClick={() => void waitForCustomer()} size="sm" variant="ghost">Aguardar cliente</Button>}
                  {["EM_ATENDIMENTO", "AGUARDANDO_CLIENTE"].includes(conversation.status) && canChangeConversation && <Button onClick={() => setActionModal({ kind: "pending", conversation })} size="sm" variant="ghost">Pendente</Button>}
                  {!isClosed && (manager || conversationIsMine) && conversation.responsavelId !== null && <Button onClick={() => setActionModal({ kind: "queue", conversation })} size="sm" variant="ghost">Devolver à fila</Button>}
                  {!isClosed && canChangeConversation && <IconButton aria-label="Encerrar conversa" onClick={() => setActionModal({ kind: "close", conversation })} title="Encerrar conversa"><CheckCircle2 size={15} /></IconButton>}
                  {isClosed && canChangeConversation && <Button disabled={busy} onClick={() => void reopenConversation()} size="sm">Reabrir</Button>}
            ...
            {!canReplyDirectly && <div className="border-b border-sky-200 bg-sky-50 px-4 py-2 text-[11px] text-sky-800">Formulário do Site não possui resposta direta. Entre em contato pelo telefone ou e-mail informado.</div>}
            {leaseFromOther && <div className="border-b border-amber-200 bg-amber-50 px-4 py-2 text-[11px] text-amber-800"><strong>{lease?.nome ?? "Outro usuário"}</strong> está respondendo esta conversa. Reserva até {formatCommunicationTime(lease?.expiraEm)}.</div>}
...
function ConversationListItem({ active, currentUserId, item, onClick }: { active: boolean; currentUserId: number; item: CommunicationConversation; onClick: () => void }) {
  ...
  return <button aria-current={active ? "true" : undefined} className={`w-full border-b border-[var(--border-default)] px-3 py-3 text-left transition-colors hover:bg-[var(--bg-muted)] focus-visible:relative focus-visible:z-10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--focus-ring)] ${active ? "bg-[var(--bg-muted)] shadow-[inset_3px_0_0_var(--primary)]" : "bg-[var(--bg-surface)]"}`} onClick={onClick} type="button"><div className="flex items-start gap-2.5"><span className="relative flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-[var(--border-default)] bg-[var(--surface-subtle)] text-[10px] font-semibold">{initials(name)}{item.naoLidas > 0 && <span aria-label={`${item.naoLidas} mensagens não lidas`} className="absolute -right-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-[var(--primary)] px-1 text-[9px] font-bold text-white">{Math.min(item.naoLidas, 99)}</span>}</span><span className="min-w-0 flex-1"><span className="flex items-center justify-between gap-2"><span className="truncate text-xs font-semibold text-[var(--text-primary)]">{name}</span><span className="shrink-0 text-[10px] tabular-nums text-[var(--text-muted)]">{formatCommunicationTime(item.ultimaMensagemEm ?? item.updatedAt)}</span></span><span className="mt-0.5 flex items-center gap-1.5 text-[10px] text-[var(--text-muted)]"><span>{channelLabel(item.canalIntegracao.tipo, item.canalIntegracao.nome)}</span><span aria-hidden="true">·</span><span className={item.responsavel ? "" : "text-[var(--warning)]"}>{item.responsavel?.nome ?? "Fila compartilhada"}</span></span><span className="mt-1 line-clamp-2 text-[11px] leading-4 text-[var(--text-secondary)]">{item.ultimaMensagem?.texto ?? "Sem mensagens"}</span><span className="mt-2 flex flex-wrap items-center gap-1.5"><ConversationStatusBadge status={item.status} />{item.sla && <ConversationSlaBadge sla={item.sla} />}{otherLease && <Badge variant="warning">{item.reservaResposta?.nome ?? "Equipe"} respondendo</Badge>}</span></span></div></button>;
}
...
function ChannelBadge({ conversation }: { conversation: CommunicationConversation }) {
  ...
  return <Badge className="gap-1" variant="neutral">{icon}{label}{conversation.canalIntegracao.tipo !== "SITE_FORM" && conversation.canalIntegracao.modoTeste ? " · simulado" : ""}</Badge>;
}
