  ...
  return (
    <nav {...props} aria-label="Paginação" className={cx("flex min-w-0 items-center justify-between gap-3 border-t border-[var(--border-default)] px-4 py-3", className)}>
      ...
      <p aria-live="polite" className="min-w-0 text-center text-[11px] tabular-nums text-[var(--text-muted)]">
        <span className="hidden whitespace-nowrap sm:inline">Página <span className="font-semibold text-[var(--text-secondary)]">{currentPage}</span> de {safeTotalPages}{visibleCount !== undefined && total !== undefined && <> · <span className="font-semibold text-[var(--text-secondary)]">{visibleCount}</span> de {total} {itemLabel}</>}</span>
        <span className="whitespace-nowrap sm:hidden">{currentPage}/{safeTotalPages}{visibleCount !== undefined && total !== undefined && <> · {visibleCount}/{total}</>}</span>
      </p>
