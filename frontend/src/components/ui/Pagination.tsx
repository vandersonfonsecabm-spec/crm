...
export function Pagination({ className, disabled = false, itemLabel = "registros", nextLabel = "Próxima", onPageChange, page, previousLabel = "Anterior", total, totalPages, visibleCount, ...props }: PaginationProps) {
  ...
  return (
    <nav {...props} aria-label="Paginação" className={cx("flex min-w-0 items-center justify-between gap-3 border-t border-[var(--border-default)] px-4 py-3", className)}>
      <Button aria-label={`Ir para ${previousLabel.toLowerCase()}`} disabled={disabled || !hasPrevious} leftIcon={<ChevronLeft size={13} />} onClick={() => onPageChange(Math.max(1, page - 1))} size="sm" variant="secondary">{previousLabel}</Button>
      ...
      <Button aria-label={`Ir para ${nextLabel.toLowerCase()}`} disabled={disabled || !hasNext} onClick={() => onPageChange(Math.min(safeTotalPages, page + 1))} rightIcon={<ChevronRight size={13} />} size="sm" variant="secondary">{nextLabel}</Button>
    </nav>
  );
}
