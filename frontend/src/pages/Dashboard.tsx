            ...
            <div className="space-y-4">
              {activePage === "clientes" && (
                  ...
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody>
                      {paginatedClients.map((client) => (
                        <tr key={client.id} className={`border-t border-white/5 transition-all duration-200 hover:bg-white/[0.025] ${selectedId === client.id ? "bg-white/[0.03] shadow-[inset_2px_0_0_rgba(148,163,184,0.28)]" : ""}`}>
                          <td className="px-3 py-2.5">
                            <button onClick={() => setSelectedId(client.id)} className="text-left">
                              <div className="flex items-center gap-2">
                              ...
                              <div className="mt-1 flex flex-wrap gap-1">
                                {client.tags.map((tag) => (
                                  <span key={tag} className={`rounded-full border px-1.5 py-0.5 text-[9px] ${tagClass(tag)}`}>{tag}</span>
                                ))}
                              </div>
                            </button>
                          </td>
                          ...
                          <td className="px-3 py-2.5">
                            <span className={`rounded-full border px-2 py-1 text-[11px] ${statusClass(client.status)}`}>{client.status}</span>
                          </td>
                          ...
                          <td className="px-3 py-2.5">
                            <div className="flex items-center gap-1">
                              <button title="Favoritar" onClick={() => toggleFavorite(client.id)} className="rounded-lg p-1.5 text-slate-300 transition hover:bg-white/10 hover:text-amber-200"><Star size={14} /></button>
                              <button title="Marcar como quente" onClick={() => toggleHot(client.id)} className="rounded-lg p-1.5 text-slate-300 transition hover:bg-white/10 hover:text-rose-200"><Flame size={14} /></button>
                              <button title="Editar cliente" onClick={() => setEditing(client)} className="rounded-lg p-1.5 text-slate-300 transition hover:bg-white/10 hover:text-sky-200"><Edit3 size={14} /></button>
                              <button title="Copiar telefone" onClick={() => copyText(client.phone, "Telefone copiado.")} className="rounded-lg p-1.5 text-slate-300 transition hover:bg-white/10 hover:text-emerald-200"><Phone size={14} /></button>
                              <a title="Abrir WhatsApp" href={`https://wa.me/${client.phone}?text=${encodeURIComponent(whatsappMessage(client))}`} target="_blank" rel="noreferrer" className="rounded-lg p-1.5 text-slate-300 transition hover:bg-white/10 hover:text-emerald-200"><MessageCircle size={14} /></a>
                            </div>
                          </td>
                        </tr>
                      ))}
                ...
                <div className="flex items-center justify-between border-t border-white/10 bg-black/10 px-3 py-2">
                  <button onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={page === 1} className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-slate-300 transition hover:bg-white/10 disabled:opacity-40">
                    Anterior
                  </button>

                  <p className="text-[11px] text-slate-500">
                    Mostrando <span className="font-semibold text-slate-300">{paginatedClients.length}</span> de {filteredClients.length}
                  </p>

                  <button onClick={() => setPage((current) => Math.min(totalPages, current + 1))} disabled={page === totalPages} className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-slate-300 transition hover:bg-white/10 disabled:opacity-40">
                    Próxima
                  </button>
                </div>
                </div>
