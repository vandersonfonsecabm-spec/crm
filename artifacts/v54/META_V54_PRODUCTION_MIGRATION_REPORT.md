...
- Canonical Prisma migration execution completed successfully and post-migration invariants prove a complete final state. This report does not assert an engine-level transaction guarantee for the already-applied multi-statement migration; the already-applied migration file was not rewritten, preserving its recorded checksum. Future multi-statement PostgreSQL migrations must include an explicit transaction or equivalent failure-atomicity proof.
