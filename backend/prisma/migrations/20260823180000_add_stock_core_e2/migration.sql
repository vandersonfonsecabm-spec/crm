...
CREATE TABLE "CapacidadeFonteEstoque" (
    ...
    "observadaEm" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE UNIQUE INDEX "LocalEstoque_empresaId_id_key" ON "LocalEstoque"("empresaId", "id");
...
CREATE TABLE "LoteEstoque" (
    ...
    CONSTRAINT "stock_lot_validade_precision_ck" CHECK (
      ("precisaoValidade" = 'DAY' AND "validadeEm" GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]') OR
      ("precisaoValidade" = 'MONTH' AND "validadeEm" GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]') OR
      ("precisaoValidade" = 'YEAR' AND "validadeEm" GLOB '[0-9][0-9][0-9][0-9]') OR
