...
async function polymorphicCount(client, kind) {
  ...
  const result = (await queryRows(client, `SELECT ${count(`e."entidadeTipo" = 'LEAD' AND e."leadId" IS NULL AND e."negocioId" IS NULL AND e."resumoJson" LIKE '%"sourceType":"PILOT_SYNTHETIC"%' AND e."resumoJson" LIKE '%"synthetic":true%'`)} AS synthetic, ${count(`NOT (e."entidadeTipo" = 'LEAD' AND e."leadId" IS NULL AND e."negocioId" IS NULL AND e."resumoJson" LIKE '%"sourceType":"PILOT_SYNTHETIC"%' AND e."resumoJson" LIKE '%"synthetic":true%') AND e."entidadeTipo" = 'LEAD' AND l."id" IS NULL`)} AS orphaned_lead, ${count(`e."entidadeTipo" = 'LEAD' AND l."id" IS NOT NULL AND l."empresaId" <> e."empresaId"`)} AS crossed_lead, ${count(`e."entidadeTipo" = 'LEAD' AND (e."leadId" IS NULL OR e."leadId" <> e."entidadeId" OR e."negocioId" IS NOT NULL)`)} AS incoherent_lead, ${count(`e."entidadeTipo" = 'NEGOCIO' AND n."id" IS NULL`)} AS orphaned_business, ${count(`e."entidadeTipo" = 'NEGOCIO' AND n."id" IS NOT NULL AND n."empresaId" <> e."empresaId"`)} AS crossed_business, ${count(`e."entidadeTipo" = 'NEGOCIO' AND (e."negocioId" IS NULL OR e."negocioId" <> e."entidadeId" OR e."leadId" IS NOT NULL)`)} AS incoherent_business FROM "AutomacaoExecucao" e LEFT JOIN "Lead" l ON l."id" = e."entidadeId" AND e."entidadeTipo" = 'LEAD' LEFT JOIN "Negocio" n ON n."id" = e."entidadeId" AND e."entidadeTipo" = 'NEGOCIO'`))[0] || {};
