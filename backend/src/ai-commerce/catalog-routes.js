...
function mountCatalogRoutes({ app, prisma, authenticate, requireRole, enabledForTenant = () => true, env = process.env, clock, policy = {} } = {}) {
  ...
  app.get("/catalogo-comercial", ...readGuard, route(async (req, res, empresaId) => res.json(await catalog.list({ empresaId, cursor: req.query.cursor, limit: req.query.limit, includeHidden: isManager(req), category: req.query.category, brand: req.query.brand }))));
  app.get("/catalogo-comercial/produtos", ...readGuard, route(async (req, res, empresaId) => res.json(await catalog.list({ empresaId, cursor: req.query.cursor, limit: req.query.limit, includeHidden: isManager(req), category: req.query.category, brand: req.query.brand }))));
