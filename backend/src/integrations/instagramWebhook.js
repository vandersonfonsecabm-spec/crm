...
const INSTAGRAM_WEBHOOK_PATH = "/webhooks/instagram";
const MAX_WEBHOOK_BODY_BYTES = 1024 * 1024;

function mountInstagramWebhookRoutes({ app, env = process.env, processWebhook = processorNotReady }) {
  app.get(INSTAGRAM_WEBHOOK_PATH, (req, res) => handleVerification(req, res, env));
  app.post(
  ...
  app.use(INSTAGRAM_WEBHOOK_PATH, instagramWebhookErrorHandler);
}

function handleVerification(req, res, env) {
  const configuredToken = env.INSTAGRAM_WEBHOOK_VERIFY_TOKEN;
  if (!isEnabled(env.INSTAGRAM_INTEGRATION_ENABLED) || !hasConfiguredSecret(configuredToken)) {
...
function inboundGate(req, res, next, env) {
  if (
    !isEnabled(env.INSTAGRAM_INTEGRATION_ENABLED)
    || !isEnabled(env.INSTAGRAM_INBOUND_ENABLED)
...
async function handleWebhook(req, res, env, processWebhook) {
  ...
  if (!isValidHmacSignature(req.body, signature, env.INSTAGRAM_APP_SECRET)) {
...
function instagramWebhookErrorHandler(error, req, res, next) {
