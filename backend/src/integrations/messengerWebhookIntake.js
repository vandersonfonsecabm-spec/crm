...
const EVENT_TYPES = Object.freeze({
  TEXT: "MESSENGER_MESSAGE_RECEIVED",
  STATUS: "MESSENGER_STATUS",
  MEDIA_UNSUPPORTED: "MESSENGER_ATTACHMENT_UNSUPPORTED",
  IGNORED: "MESSENGER_IGNORED",
});
...
function createMessengerWebhookIntake({ prisma, clock = () => new Date() }) {
  if (!prisma) throw new Error("Prisma e obrigatorio para o intake Messenger.");
  if (typeof clock !== "function") throw new Error("Relogio invalido para o intake Messenger.");

  return async function processMessengerWebhook(payload, { env = process.env } = {}) {
...
function parseAtomicEvents(payload) {
  if (!isObject(payload) || payload.object !== "page" || !Array.isArray(payload.entry)) {
  ...
  for (const entry of payload.entry) {
    ...
    const messengerPageId = requiredIdentifier(entry.id, MAX_ID_LENGTH);
    if (!messengerPageId || !Array.isArray(entry.messaging)) {
      throw intakeError(400, "WEBHOOK_PAYLOAD_INVALID");
    }
    ...
    for (const event of entry.messaging) {
      items.push(parseMessagingEvent(event, messengerPageId));
    }
  }
...
function parseMessagingEvent(event, messengerPageId) {
  ...
  if (
    ...
    || !recipientId
    || !timestamp
    || (isEcho ? senderId !== messengerPageId : recipientId !== messengerPageId)
  ) {
    throw intakeError(400, "WEBHOOK_PAYLOAD_INVALID");
  }
  ...
  if (isObject(event.message)) {
    ...
    } else if (Array.isArray(event.message.attachments) && event.message.attachments.length > 0) {
...
function requireSingleIntegrationIdentity(items) {
  ...
  if (items.some((item) => item.messengerPageId !== first.messengerPageId)) {
    throw intakeError(400, "WEBHOOK_PAYLOAD_INVALID");
  }
  return { messengerPageId: first.messengerPageId };
}

async function mapIntegration(prisma, { messengerPageId }, env) {
  let matches;
  try {
    const global = readGlobalMessengerConfiguration(env);
...
async function readCapabilities(prisma, empresaId, env) {
  if (env.MESSENGER_INTEGRATION_ENABLED !== "true" || env.MESSENGER_INBOUND_ENABLED !== "true") {
  ...
  return {
    integrationEnabled: enabled.has("MESSENGER_INTEGRATION"),
    inboundEnabled: enabled.has("MESSENGER_INTEGRATION") && enabled.has("MESSENGER_INBOUND"),
  };
}
...
async function touchActiveChannel(tx, integration, receivedAt, allowRetry = true) {
  const channel = await tx.canalIntegracao.findFirst({
    where: {
      ...
      ativo: true,
      status: "ATIVO",
      messengerPageId: integration.messengerPageId,
...
function sameAtomicItem(left, right) {
  return left.externalEventId === right.externalEventId
    && left.messengerPageId === right.messengerPageId
...
function intakeError(status, code) {
  ...
  error.name = "MessengerWebhookIntakeError";
  error.status = status;
  error.code = code;
  return error;
}
...
function isIntakeError(error) {
  return error?.name === "MessengerWebhookIntakeError";
}
