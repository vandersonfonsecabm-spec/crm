...
function createMessengerInboundLifecycleService({
  prisma,
  env = process.env,
  logger = console,
  clock = () => new Date(),
} = {}) {
  if (!prisma) throw new Error("Prisma obrigatorio para lifecycle Messenger.");
  ...
  async function changeState({
    ...
    if (!ACTIONS.has(action)) {
      throw lifecycleError(422, "MESSENGER_LIFECYCLE_INVALID", "Acao de lifecycle invalida.");
    }
...
async function loadContext(client, tenantId, env) {
  ...
  return {
    ...
    capabilities: {
      ...
      inbound: features.get(MESSENGER_CAPABILITY_KEYS.INBOUND) === true,
    },
    global,
  };
}
...
function deriveState(context) {
  ...
  if (!channel) {
    return realChannels.length > 0
      ? MESSENGER_OPERATIONAL_STATUS.ERROR
      : MESSENGER_OPERATIONAL_STATUS.NOT_CONFIGURED;
  }
  if (!hasEssentialIdentity(channel)) return MESSENGER_OPERATIONAL_STATUS.NOT_CONFIGURED;
  if (
  ...
  if (channel.ativo === false && channel.status === "INATIVO") {
    return channel.verifiedAt || channel.connectedAt
      ? MESSENGER_OPERATIONAL_STATUS.PAUSED
      : MESSENGER_OPERATIONAL_STATUS.CONFIGURED_INACTIVE;
  }
...
function unavailableStatus() {
  return {
    ...
    tipo: MESSENGER_CHANNEL_TYPE,
    name: null,
    messengerPageIdMasked: null,
    messengerPageNameMasked: null,
...
function nextRequirement(state) {
  if (state === MESSENGER_OPERATIONAL_STATUS.NOT_CONFIGURED) {
    return "PROVISION_MESSENGER_INBOUND";
  }
  if (state === MESSENGER_OPERATIONAL_STATUS.CONFIGURED_INACTIVE) {
    return "ACTIVATE_MESSENGER_INBOUND";
  }
  if (state === MESSENGER_OPERATIONAL_STATUS.WAITING_META_AUTH) {
    return "CONFIGURE_META_CALLBACK";
  }
  if (state === MESSENGER_OPERATIONAL_STATUS.PAUSED) return "REACTIVATE_MESSENGER_INBOUND";
  if (state === MESSENGER_OPERATIONAL_STATUS.ERROR) return "RECONCILE_MESSENGER_CHANNEL";
  if (state === MESSENGER_OPERATIONAL_STATUS.UNAVAILABLE) return "RETRY_STATUS";
  return null;
}

function validateLifecyclePayload(body) {
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    throw lifecycleError(422, "MESSENGER_LIFECYCLE_INVALID", "Payload invalido.");
  }
  const unknown = Object.keys(body).filter((key) => !ALLOWED_ACTION_FIELDS.has(key));
  if (unknown.length) {
    throw lifecycleError(
      422,
      "MESSENGER_LIFECYCLE_INVALID",
  ...
  if (!reason) {
    throw lifecycleError(422, "MESSENGER_REASON_REQUIRED", "reason e obrigatorio.");
  }
...
function normalizeExpectedUpdatedAt(value) {
  ...
  if (Number.isNaN(parsed.getTime()) || parsed.toISOString() !== value) {
    throw lifecycleError(422, "MESSENGER_LIFECYCLE_INVALID", "expectedUpdatedAt invalido.");
  }
  return parsed;
}
...
function normalizeReason(value) {
  if (typeof value !== "string" || value.length > MAX_REASON_LENGTH) {
    throw lifecycleError(422, "MESSENGER_LIFECYCLE_INVALID", "reason invalido.");
  }
...
function targetForAction(action) {
  if (action === "PAUSE") {
    return {
      ...
      status: "INATIVO",
      capabilityChanges: [
        { featureKey: MESSENGER_CAPABILITY_KEYS.INBOUND, enabled: false },
      ],
    };
  }
  return {
    ...
    status: "ATIVO",
    capabilityChanges: [
      { featureKey: MESSENGER_CAPABILITY_KEYS.INTEGRATION, enabled: true },
      { featureKey: MESSENGER_CAPABILITY_KEYS.INBOUND, enabled: true },
    ],
  };
}
...
function matchesTarget(context, target) {
  ...
  return target.capabilityChanges.every(({ featureKey, enabled }) => {
    const current = featureKey === MESSENGER_CAPABILITY_KEYS.INTEGRATION
...
function requireCanonicalChannel(context, globalConfig) {
  ...
  if (!channel && realChannels.length === 0) {
    throw lifecycleError(
      404,
      "MESSENGER_CHANNEL_NOT_FOUND",
      "Canal Messenger real nao encontrado.",
    );
  }
  if (
    !channel
...
async function assertIdentityAvailable(tx, channel) {
  ...
  if (conflict) {
    throw lifecycleError(
      ...
      "Identidade Messenger ja vinculada ou inconsistente.",
    );
  }
}
...
function assertPersistedStateIsCoherent(context, action) {
  ...
  if (!capabilitiesCoherent || !lifecycleCoherent) {
    throw lifecycleError(
      409,
      "MESSENGER_CHANNEL_STATE_INVALID",
...
function readInboundRuntimeConfiguration(env) {
  const configuration = readGlobalMessengerConfiguration(env);
  if (
    ...
    throw new Error("Messenger inbound indisponivel.");
  }
...
function hasEssentialIdentity(channel) {
  return Boolean(
    channel
      && typeof channel.messengerPageId === "string"
      && channel.messengerPageId.length > 0,
  );
}
...
function sanitizeFailureCode(value) {
  return typeof value === "string" && /^[A-Z0-9_]{1,80}$/.test(value)
    ? value
    : value ? "MESSENGER_OPERATIONAL_ERROR" : null;
}
...
function emitLifecycleAudit(logger, {
  ...
  try {
    output.call(logger, JSON.stringify({
      event: "messenger_inbound_lifecycle",
      timestamp: clock().toISOString(),
      service: "platform-messenger-lifecycle",
...
function lifecycleError(status, code, message) {
  ...
  error.name = "MessengerInboundLifecycleError";
  error.status = status;
  error.code = code;
  return error;
}
...
function isLifecycleError(error) {
  return error?.name === "MessengerInboundLifecycleError";
}
