...
const {
  ...
  canonicalStringify,
  derivedEventId,
} = require("./messengerWebhookIntake");
...
async function processMessengerWebhookEvent({ prisma, eventoWebhookId, lease = null }) {
  if (!prisma || !Number.isInteger(eventoWebhookId) || eventoWebhookId < 1) {
    throw processingError("MESSENGER_EVENT_PROCESSOR_INVALID_INPUT");
  }
...
async function processWithUniqueRecovery(prisma, eventoWebhookId, allowUniqueRecovery, lease) {
  try {
    ...
    throw processingError("MESSENGER_EVENT_PROCESSING_UNAVAILABLE");
  }
}
...
async function processTransaction(tx, eventoWebhookId, lease) {
  ...
  if (event.statusProcessamento === PROCESSED_STATUS) {
    if (!event.processadoEm) throw processingError("MESSENGER_EVENT_STATE_INVALID");
  ...
  if (lease) {
    if (!isMatchingLease(event, lease)) {
      throw processingError("MESSENGER_EVENT_LEASE_LOST");
    }
  } else {
    if (event.statusProcessamento !== PROCESSABLE_STATUS || event.processadoEm !== null) {
      throw processingError("MESSENGER_EVENT_STATE_INVALID");
    }
    ...
    if (claim.count !== 1) {
      ...
      throw processingError("MESSENGER_EVENT_CONCURRENCY_CONFLICT");
    }
  }
...
async function loadEvent(client, eventoWebhookId) {
  ...
  if (!event) throw processingError("MESSENGER_EVENT_NOT_FOUND");
  return event;
}
...
function validateEventOwnership(event) {
  let global;
  try {
    global = readGlobalMessengerConfiguration(process.env);
  } catch {
  ...
  if (event.provedor !== PROVIDER || !Object.values(EVENT_TYPES).includes(event.tipoEvento)) {
    throw processingError("MESSENGER_EVENT_UNSUPPORTED");
  }
  if (
    !event.empresa?.ativo
...
async function requireProcessingCapabilities(prisma, empresaId) {
  if (
    process.env.MESSENGER_INTEGRATION_ENABLED !== "true"
    || process.env.MESSENGER_INBOUND_ENABLED !== "true"
  ) {
    throw processingError("MESSENGER_EVENT_PROCESSING_NOT_AVAILABLE");
  }
  ...
  if (!enabled.has("MESSENGER_INTEGRATION") || !enabled.has("MESSENGER_INBOUND")) {
    throw processingError("MESSENGER_EVENT_PROCESSING_NOT_AVAILABLE");
  }
}
...
function validateAtomicPayload(event) {
  if (
    ...
    throw processingError("MESSENGER_EVENT_PAYLOAD_INVALID");
  }
  const hash = crypto.createHash("sha256").update(event.payloadJson, "utf8").digest("hex");
  if (hash !== event.payloadHash) throw processingError("MESSENGER_EVENT_PAYLOAD_INTEGRITY_FAILED");
  ...
  if (
    ...
    || payload.messengerPageId !== event.canalIntegracao.messengerPageId
  ...
  if (
    ...
    || !recipientId
    || (isEcho
      ? senderId !== payload.messengerPageId
      : recipientId !== payload.messengerPageId)
  ) {
    throw processingError("MESSENGER_EVENT_PAYLOAD_INVALID");
  }
...
async function reserveActiveChannel(tx, event) {
  const reserved = await tx.canalIntegracao.updateMany({
    where: {
      ...
      ativo: true,
      status: "ATIVO",
      messengerPageId: event.canalIntegracao.messengerPageId,
      metaAppId: event.canalIntegracao.metaAppId,
      providerEnvironment: event.canalIntegracao.providerEnvironment,
    },
    data: { ativo: true },
  });
  if (reserved.count !== 1) throw processingError("MESSENGER_EVENT_INTEGRATION_INVALID");
}
...
async function verifyTerminalEvent(tx, event) {
  ...
  if (message) throw processingError("MESSENGER_PROCESSED_EVENT_INCONSISTENT");
}
...
async function resolveContact(tx, event, atomic) {
  ...
  if (existing) {
    if (
      ...
      throw processingError("MESSENGER_CONTACT_INTEGRITY_CONFLICT");
    }
    return existing;
  }
...
async function linkContactToClient(tx, event, contact, client) {
  if (client.empresaId !== event.empresaId) throw processingError("MESSENGER_CLIENT_INTEGRITY_CONFLICT");
  if (contact.clienteId === client.id) return contact;
  if (contact.clienteId !== null) throw processingError("MESSENGER_CONTACT_CLIENT_CONFLICT");
  ...
  if (updated.count !== 1) throw processingError("MESSENGER_CONTACT_CLIENT_CONFLICT");
...
async function findSingleActiveConversation(tx, event, contact) {
  ...
  if (conversations.length > 1) throw processingError("MESSENGER_CONVERSATION_AMBIGUOUS");
...
async function resolveLead(tx, event, client, activeConversation) {
  if (activeConversation?.leadId) {
    ...
    if (!linked || !ACTIVE_LEAD_STATUSES.includes(linked.status)) {
      throw processingError("MESSENGER_LEAD_INTEGRITY_CONFLICT");
    }
    return linked;
  }
  ...
  if (candidates.length > 1) throw processingError("MESSENGER_LEAD_AMBIGUOUS");
  ...
  await createAutomationService({ prisma: tx }).enqueueLeadCreated({
    ...
    empresaId: event.empresaId,
    leadId: lead.id,
    originalEventId: `messenger:${event.id}`,
...
async function resolveConversation(tx, event, contact, client, lead, activeConversation, messageTime) {
  ...
  if (activeConversation) {
    if (
      ...
      throw processingError("MESSENGER_CONVERSATION_INTEGRITY_CONFLICT");
    }
    ...
    if (linked.count !== 1) throw processingError("MESSENGER_CONVERSATION_INTEGRITY_CONFLICT");
  ...
  if (occupied) throw processingError("MESSENGER_CONVERSATION_INTEGRITY_CONFLICT");
...
async function verifyProcessedChain(tx, event, atomic) {
  ...
  if (
    ...
    throw processingError("MESSENGER_PROCESSED_EVENT_INCONSISTENT");
  }
...
function assertMessageEquivalent(message, event, contact, conversation, atomic) {
  if (
    ...
    throw processingError("MESSENGER_MESSAGE_IDEMPOTENCY_CONFLICT");
  }
}
...
function parseExternalTimestamp(value) {
  ...
  if (typeof normalized !== "string" || !/^\d{1,20}$/.test(normalized)) {
    throw processingError("MESSENGER_EVENT_TIMESTAMP_INVALID");
  }
  ...
  if (Number.isNaN(date.getTime())) throw processingError("MESSENGER_EVENT_TIMESTAMP_INVALID");
  return date;
}
...
function processingError(code) {
  const error = new Error("Evento Messenger nao processado.");
  error.name = "MessengerWebhookProcessingError";
  error.code = code;
  return error;
}

function isProcessingError(error) {
  return error?.name === "MessengerWebhookProcessingError";
}
