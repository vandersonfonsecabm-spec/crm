...
test("API oficial exige pin do servico e do target PostgreSQL antes de Prisma", () => {
  const postgresUrl = "postgresql://pin-test:pin-test@postgres.internal:5432/crm_target_pin";
  const baseEnv = {
    NODE_ENV: "production",
    RAILWAY_SERVICE_ID: "16de1b91-7dcb-46b4-9231-1c3e2c3e5a92",
    RAILWAY_DEPLOYMENT_ID: "deployment-target-pin",
    RAILWAY_PROJECT_ID: "ddfbf66c-e274-47b1-9493-286232d2f426",
    RAILWAY_ENVIRONMENT_ID: "e18f76b1-e38f-468e-91fe-1eff6db9a5f8",
    CRM_DATABASE_PROVIDER: "postgresql",
    CRM_DATABASE_SERVICE_ID: "e9d8a6b8-507b-45fb-92a8-3ab016f865a2",
    CRM_DATABASE_TARGET_FINGERPRINT: databaseTargetFingerprint(postgresUrl),
    POSTGRES_DATABASE_URL: postgresUrl,
  };
  const options = {
    backendDirectory,
    expectedServiceId: baseEnv.RAILWAY_SERVICE_ID,
    prismaCliPath: resolvePrismaCli(backendDirectory),
    preparePrismaRuntime: ({ env, provider }) => ({ env, provider, schemaPath: path.join(sourcePrismaDirectory, "schema.prisma") }),
  };

  assert.doesNotThrow(() => validateRailwayEnvironment({ ...options, env: baseEnv }));
  assert.throws(
    () => validateRailwayEnvironment({ ...options, env: { ...baseEnv, CRM_DATABASE_SERVICE_ID: "wrong-database-service" } }),
    { code: "RAILWAY_DATABASE_SERVICE_MISMATCH" },
  );
  assert.throws(
    () => validateRailwayEnvironment({ ...options, env: { ...baseEnv, POSTGRES_DATABASE_URL: "postgresql://pin-test:pin-test@postgres.internal:5432/other_database" } }),
    { code: "RAILWAY_DATABASE_TARGET_MISMATCH" },
  );
  assert.throws(
    () => validateRailwayEnvironment({ ...options, env: { ...baseEnv, DATABASE_URL: "postgresql://pin-test:pin-test@postgres.internal:5432/other_database" } }),
    { code: "RAILWAY_DATABASE_URL_TARGET_MISMATCH" },
  );
});
