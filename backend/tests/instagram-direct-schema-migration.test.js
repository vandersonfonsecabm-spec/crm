...
const migrationName = "20260730160000_add_instagram_direct_schema_foundation";
const auditDir = path.join(
  requiredEnv("CRM_PRISMA_TEST_RUN_DIR"),
  "instagram-direct-schema-migration",
);
...
test("migration Instagram e aditiva e preserva dados SQLite existentes", async () => {
  ...
  try {
    ...
    assert.ok(indexes.some((item) => (
      item.name === "CanalIntegracao_instagramBusinessAccountId_key"
