...
test("Release B1 protege tenant, RBAC, fila, atribuicoes, mensagens e idempotencia", async () => {
  ...
  const archivedAssignmentLead = await request("POST", "/leads", { clienteId: clientA2.id }, adminA.token);
  assert.equal(archivedAssignmentLead.status, 201);
  await prisma.cliente.update({ where: { id: clientA2.id }, data: { arquivadoEm: new Date(), status: "Arquivado" } });
  assert.equal((await request("POST", `/leads/${archivedAssignmentLead.body.id}/assumir`, {}, sellerA.token)).status, 404);
  assert.equal((await request("POST", `/leads/${leadSellerA.body.id}/atribuir`, { responsavelId: sellerA.usuarioId }, managerA.token)).status, 404);
  assert.equal((await request("POST", `/leads/${leadSellerA.body.id}/devolver-fila`, { motivo: "Cliente arquivado" }, sellerA2.token)).status, 404);
  assert.equal((await request("GET", "/conversas/resumo", undefined, adminA.token)).body.pendentes, 0);
  assert.equal((await request("GET", "/conversas", undefined, adminA.token)).body.pagination.total, 0);
