...
if (!postgres) {
  ...
  databasePath = path.join(runDir, "messenger-inbound-provisioning", `provisioning-${process.pid}.db`);
...
Object.assign(process.env, {
  NODE_ENV: "test",
  JWT_SECRET: "messenger-inbound-provisioning-test-secret-with-entropy",
  JWT_EXPIRES_IN: "1h",
  ...
  MESSENGER_APP_SECRET: "messenger-provisioning-test-app-secret",
  MESSENGER_WEBHOOK_VERIFY_TOKEN: "messenger-provisioning-test-verify-token",
  MESSENGER_META_APP_ID: "000GLOBAL_MESSENGER_APP",
  MESSENGER_PROVIDER_ENVIRONMENT: "MESSENGER_TEST",
});
...
test("provisiona Messenger inbound inativo com RBAC, allowlist e resposta sanitizada", async () => {
  const operator = await register("Operadora Messenger", "Operadora", uniqueEmail("operator"));
  const target = await register("Tenant Messenger", "Admin Alvo", uniqueEmail("target"));
  ...
  const environment = process.env.MESSENGER_PROVIDER_ENVIRONMENT;
  ...
  process.env.MESSENGER_PROVIDER_ENVIRONMENT = "ambiente invalido";
  response = await put(target.empresaId, validCreation(), operator.token);
  assert.equal(response.status, 503);
  process.env.MESSENGER_PROVIDER_ENVIRONMENT = environment;
  ...
  for (const field of [
    ...
    assert.equal(forbidden.body.codigo, "MESSENGER_PROVISIONING_INVALID", field);
  }
  const unmaskedUsername = await put(target.empresaId, {
    ...validCreation(),
    messengerPageNameMasked: "@handle_completo",
  }, operator.token);
  assert.equal(unmaskedUsername.status, 422);
  assert.equal(unmaskedUsername.body.codigo, "MESSENGER_PROVISIONING_INVALID");

  const testChannel = await prisma.canalIntegracao.create({
    data: {
      empresaId: target.empresaId,
      tipo: "MESSENGER_META",
      nome: "Messenger de teste",
      chaveInterna: "messenger-meta-test",
      status: "MODO_TESTE",
      modoTeste: true,
      ativo: true,
      messengerPageId: `TEST_MESSENGER_${suffix}`,
    },
  });
  ...
  try {
    response = await put(target.empresaId, {
      ...
      reason: [
        ...
        "messengerPageId=000MESSENGER_F1",
        "accessTokenRef=env:MESSENGER_TOKEN",
        "telefone=+55 11 99999-0000",
      ].join("\n"),
    }, operator.token, { "x-correlation-id": "messenger-create" });
  } finally {
  ...
  assert.equal(response.body.tipo, "MESSENGER_META");
  assert.equal(response.body.ativo, false);
  assert.equal(response.body.status, "INATIVO");
  assert.equal(response.body.name, "Messenger inbound real");
  assert.equal(response.body.messengerPageIdMasked, "****R_F1");
  assert.equal(response.body.messengerPageNameMasked, "@conta_****");
  ...
  assert.equal(response.body.nextRequirement, "ACTIVATE_MESSENGER_INBOUND");
  assert.equal(JSON.stringify(response.body).includes("000MESSENGER_F1"), false);
  ...
  assert.ok(channel);
  assert.equal(channel.messengerPageId, "000MESSENGER_F1");
  ...
  assert.equal(await messengerCapabilityCount(target.empresaId), 0);
  assert.deepEqual(await effectCounts(target.empresaId), effectsBefore);

  assert.equal(auditLines.length, 1);
  const audit = JSON.parse(auditLines[0]);
  assert.equal(audit.event, "messenger_inbound_channel_provisioning");
  ...
  assert.deepEqual(audit.changedFields, ["name", "messengerPageNameMasked"]);
  assert.equal(audit.correlationId, "messenger-create");
  ...
  const testPatch = await request(
    ...
    { nome: "Messenger teste atualizado" },
    target.token,
  );
  assert.equal(testPatch.status, 200);
  assert.equal(testPatch.body.nome, "Messenger teste atualizado");
  ...
  assert.equal(status.body.messengerPageIdMasked, "****R_F1");
  assert.equal(JSON.stringify(status.body).includes("000MESSENGER_F1"), false);
  assert.deepEqual(status.body.capabilities, { integration: false, inbound: false });

  const emptyTenant = await register("Tenant Sem Messenger", "Admin Vazio", uniqueEmail("empty"));
  ...
  assert.equal(emptyStatus.body.nextRequirement, "PROVISION_MESSENGER_INBOUND");
});
...
test("preserva idempotencia, identidade imutavel e CAS sem mutacao parcial", async () => {
  ...
  const created = await put(target.empresaId, {
    ...validCreation(),
    messengerPageId: `000MESSENGER_CAS_${suffix}`,
  }, operator.token);
  assert.equal(created.status, 201);

  const replay = await put(target.empresaId, {
    name: "Messenger inbound real",
    messengerPageId: `000MESSENGER_CAS_${suffix}`,
    messengerPageNameMasked: "@conta_****",
  }, operator.token);
  assert.equal(replay.status, 200);
  assert.equal(replay.body.changed, false);
  assert.equal(await realMessengerCount(target.empresaId), 1);
  ...
  assert.equal(missingReason.body.codigo, "MESSENGER_REASON_REQUIRED");
  const missingVersion = await put(target.empresaId, {
    name: "Nome atualizado",
    reason: "Atualizar metadata",
  }, operator.token);
  assert.equal(missingVersion.status, 422);
  assert.equal(missingVersion.body.codigo, "MESSENGER_EXPECTED_UPDATED_AT_REQUIRED");

  const immutable = await put(target.empresaId, {
    messengerPageId: `OUTRA_IDENTIDADE_${suffix}`,
  }, operator.token);
  assert.equal(immutable.status, 409);
  assert.equal(immutable.body.codigo, "MESSENGER_IDENTITY_IMMUTABLE");
  ...
  assert.equal(concurrent.find((result) => result.status === 409).body.codigo, "MESSENGER_CHANNEL_CONFLICT");
  ...
  assert.equal(metadataUpdate.body.messengerPageNameMasked, "@novo_****");
  const afterMetadataUpdate = await canonicalChannel(target.empresaId);
  assert.equal(afterMetadataUpdate.messengerPageId, original.messengerPageId);
  ...
  assert.equal(stale.body.codigo, "MESSENGER_CHANNEL_CONFLICT");
...
test("fecha conflitos de tenant, slot, identidade e legado sem promover canal de teste", async () => {
  ...
  const identity = `000MESSENGER_SHARED_${suffix}`;
  ...
  assert.equal(crossTenant.body.codigo, "MESSENGER_IDENTITY_CONFLICT");
  assert.equal(JSON.stringify(crossTenant.body).includes(identity), false);
  assert.equal(await realMessengerCount(other.empresaId), 0);

  const divergentSameTenant = await put(owner.empresaId, {
    ...validCreation(),
    messengerPageId: `DIVERGENT_${suffix}`,
  }, operator.token);
  assert.equal(divergentSameTenant.status, 409);
  assert.equal(divergentSameTenant.body.codigo, "MESSENGER_IDENTITY_IMMUTABLE");
  assert.equal(await realMessengerCount(owner.empresaId), 1);
  ...
  const legacy = await prisma.canalIntegracao.create({
    data: {
      ...
      tipo: "MESSENGER_META",
      nome: "Messenger legado",
      chaveInterna: "messenger-legado-real",
      status: "INATIVO",
      modoTeste: false,
      ativo: false,
      messengerPageId: `LEGACY_MESSENGER_${suffix}`,
    },
  });
  const legacyBlocked = await put(legacyTenant.empresaId, {
    ...validCreation(),
    messengerPageId: `NEW_MESSENGER_${suffix}`,
  }, operator.token);
  assert.equal(legacyBlocked.status, 409);
  assert.equal(legacyBlocked.body.codigo, "MESSENGER_LEGACY_CHANNEL_CONFLICT");
  assert.deepEqual(await prisma.canalIntegracao.findUnique({ where: { id: legacy.id } }), legacy);
  assert.equal(await realMessengerCount(legacyTenant.empresaId), 1);
  ...
  const activeChannel = await prisma.canalIntegracao.create({
    data: {
      ...
      tipo: "MESSENGER_META",
      nome: "Messenger ativo incoerente",
      chaveInterna: "messenger-meta-inbound-real",
      status: "ATIVO",
      modoTeste: false,
      ativo: true,
      ...
      messengerPageId: `ACTIVE_MESSENGER_${suffix}`,
    },
  });
  const activeBlocked = await put(activeTenant.empresaId, {
    name: activeChannel.nome,
    messengerPageId: activeChannel.messengerPageId,
  }, operator.token);
  assert.equal(activeBlocked.status, 409);
  assert.equal(activeBlocked.body.codigo, "MESSENGER_CHANNEL_STATE_CONFLICT");
  assert.deepEqual(
  ...
  const capabilityBlocked = await put(capabilityTenant.empresaId, {
    ...validCreation(),
    messengerPageId: `CAPABILITY_MESSENGER_${suffix}`,
  }, operator.token);
  assert.equal(capabilityBlocked.status, 409);
  assert.equal(capabilityBlocked.body.codigo, "MESSENGER_CHANNEL_STATE_CONFLICT");
  assert.equal(await realMessengerCount(capabilityTenant.empresaId), 0);
  ...
  const testChannel = await prisma.canalIntegracao.create({
    data: {
      ...
      chaveInterna: "messenger-meta-test-only",
      status: "MODO_TESTE",
      modoTeste: true,
      ativo: true,
      messengerPageId: `SYNTHETIC_TEST_${suffix}`,
    },
  });
  const realCreated = await put(testOnlyTenant.empresaId, {
    ...validCreation(),
    messengerPageId: `REAL_AFTER_TEST_${suffix}`,
  }, operator.token);
  ...
  assert.equal(await realMessengerCount(testOnlyTenant.empresaId), 1);
  assert.equal(await messengerCapabilityCount(testOnlyTenant.empresaId), 0);
});
...
test("nao mascara unique inesperada e preserva commit quando auditoria falha", async () => {
  ...
  } = require("../src/platform/messengerInboundProvisioning");
  ...
  assert.equal(classifyCanalUniqueConflictTarget({
    code: "P2002",
    meta: { target: "CanalIntegracao_messengerPageId_key" },
  }), "GLOBAL_IDENTITY");
  assert.equal(classifyCanalUniqueConflictTarget({
    code: "P2002",
    meta: { target: { constraint: { fields: ["messengerPageId"] } } },
  }), "GLOBAL_IDENTITY");
  assert.equal(classifyCanalUniqueConflictTarget({
    code: "P2002",
  ...
  await assert.rejects(
    createMessengerInboundProvisioningService({
      ...
      actorUserId: 902,
      body: {
        name: "Messenger tecnico",
        messengerPageId: "MESSENGER_PUBLIC_ID_COLLISION",
  ...
  const tenant = await register("Tenant Logger Messenger", "Admin Logger", uniqueEmail("logger"));
  const body = {
    name: "Messenger com logger indisponivel",
    messengerPageId: `MESSENGER_LOGGER_${suffix}`,
    messengerPageNameMasked: "@logger_****",
    reason: [
  ...
  const service = createMessengerInboundProvisioningService({
    prisma,
  ...
  assert.equal(JSON.stringify(created).includes(body.messengerPageId), false);
  assert.equal(await realMessengerCount(tenant.empresaId), 1);
...
test("classifica corridas reais com dois PrismaClient independentes", async () => {
  ...
  const sameBody = {
    ...validCreation(),
    messengerPageId: `MESSENGER_SAME_${suffix}`,
  };
  ...
  assert.equal(await realMessengerCount(sameTenant.empresaId), 1);
  ...
  const slotRace = await runRealProvisioningRace([
    {
      ...
      body: {
        ...validCreation(),
        messengerPageId: `MESSENGER_SLOT_A_${suffix}`,
      },
    },
    {
      tenantId: slotTenant.empresaId,
      ...
      body: {
        ...validCreation(),
        messengerPageId: `MESSENGER_SLOT_B_${suffix}`,
      },
    },
  ]);
  ...
  assert.equal(await realMessengerCount(slotTenant.empresaId), 1);
  ...
  const sharedBody = {
    ...validCreation(),
    messengerPageId: `MESSENGER_CROSS_${suffix}`,
  };
  ...
  assert.ok(rejected);
  assert.equal(rejected.reason.code, "MESSENGER_IDENTITY_CONFLICT");
  assert.equal(rejected.reason.status, 409);
  assert.equal(crossRace.uniqueErrors.length, 1);
  assertKnownConstraintTarget(crossRace.uniqueErrors[0], ["messengerPageId"]);
  assert.equal(await prisma.canalIntegracao.count({
    where: { messengerPageId: sharedBody.messengerPageId },
  }), 1);
});
...
async function runRealProvisioningRace(calls) {
  ...
  try {
    ...
    const services = clients.map((client) => createMessengerInboundProvisioningService({
...
function validCreation() {
  return {
    ...
    messengerPageId: " 000MESSENGER_F1 ",
    messengerPageNameMasked: "@conta_****",
    reason: "Provisionamento controlado Messenger",
  };
}

async function register(empresaNome, adminNome, email) {
  const senha = "SenhaMessengerSegura123";
...
async function put(tenantId, body, token, headers) {
  return request(
    "PUT",
    `/platform/tenants/${tenantId}/integrations/messenger/inbound`,
    body,
    token,
    headers,
  );
...
async function realMessengerCount(empresaId) {
  return prisma.canalIntegracao.count({
    where: { empresaId, tipo: "MESSENGER_META", modoTeste: false },
  });
}

async function messengerCapabilityCount(empresaId) {
