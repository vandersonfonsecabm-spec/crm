...
if (!postgres) {
  ...
  databasePath = path.join(runDir, "instagram-inbound-lifecycle", `lifecycle-${process.pid}.db`);
...
Object.assign(process.env, {
  NODE_ENV: "test",
  JWT_SECRET: "instagram-inbound-lifecycle-test-secret-with-entropy",
  JWT_EXPIRES_IN: "1h",
...
test("lifecycle Instagram protege RBAC, allowlist, configuracao e canais nao canonicos", async () => {
  const operator = await register("Operadora Instagram Lifecycle", "Operadora", uniqueEmail("operator"));
  const target = await register("Tenant Instagram Lifecycle", "Admin Alvo", uniqueEmail("target"));
  ...
  assert.equal(response.body.nextRequirement, "PROVISION_INSTAGRAM_INBOUND");

  response = await action("activate", target.empresaId, lifecycleBody(new Date()), operator.token);
  assert.equal(response.status, 404);
  assert.equal(response.body.codigo, "INSTAGRAM_CHANNEL_NOT_FOUND");

  const testTenant = await register(
    "Tenant Teste Instagram Lifecycle",
    "Admin Teste",
  ...
  const legacyTenant = await register(
    "Tenant Legado Instagram Lifecycle",
    "Admin Legado",
    uniqueEmail("legacy"),
  );
  const legacy = await createRealChannel(legacyTenant.empresaId, {
    chaveInterna: "instagram-meta-inbound-legacy",
  });
  response = await action(
    "activate",
  ...
  assert.equal(response.body.codigo, "INSTAGRAM_LEGACY_CHANNEL_CONFLICT");
  ...
  for (const field of [
    ...
    assert.equal(response.body.codigo, "INSTAGRAM_LIFECYCLE_INVALID", field);
  }
});

test("lifecycle Instagram ativa, pausa e reativa com CAS, idempotencia e auditoria atomicos", async () => {
  const operator = await register(
    "Operadora Fluxo Instagram",
    "Operadora",
    uniqueEmail("operator-flow"),
  );
  const target = await register(
    "Tenant Fluxo Instagram",
    "Admin Alvo",
    uniqueEmail("target-flow"),
  );
  const untouched = await register(
    "Tenant Isolado Instagram",
    "Admin Isolado",
  ...
  assert.equal(response.body.instagramBusinessAccountIdMasked, maskOpaqueId(
    channel.instagramBusinessAccountId,
  ));
  assert.equal(JSON.stringify(response.body).includes(channel.instagramBusinessAccountId), false);
  ...
  assert.equal(response.status, 409);
  assert.equal(response.body.codigo, "INSTAGRAM_CHANNEL_CONFLICT");
  ...
  try {
    response = await action("activate", target.empresaId, {
      ...
      reason: `Ativar inbound ${channel.instagramBusinessAccountId} token=segredo payload=dados`,
    }, operator.token, { "x-correlation-id": "instagram-lifecycle-activate" });
  } finally {
  ...
  for (const audit of activationAudits) {
    assert.equal(audit.motivo.includes(channel.instagramBusinessAccountId), false);
    assert.equal(audit.motivo.includes("segredo"), false);
    assert.equal(audit.motivo.includes("payload=dados"), false);
  }
  assert.equal(logLines.length, 1);
  assert.equal(logLines[0].includes(channel.instagramBusinessAccountId), false);
  assert.equal(logLines[0].includes("segredo"), false);
  assert.match(logLines[0], /instagram-lifecycle-activate/);
  ...
  assert.equal(stalePause.body.codigo, "INSTAGRAM_CHANNEL_CONFLICT");
  ...
  assert.equal(staleReactivate.body.codigo, "INSTAGRAM_CHANNEL_CONFLICT");
...
test("lifecycle Instagram preserva timestamps e deriva CONNECTED e PAUSED", async () => {
  const operator = await register(
    "Operadora Estados Instagram",
    "Operadora",
    uniqueEmail("operator-states"),
  );
  const target = await register(
    "Tenant Estados Instagram",
    "Admin Alvo",
  ...
  await createFeature(target.empresaId, "INSTAGRAM_INTEGRATION", true);
  ...
  assert.equal(response.body.nextRequirement, "REACTIVATE_INSTAGRAM_INBOUND");
  channel = await prisma.canalIntegracao.findUnique({ where: { id: channel.id } });
  ...
  channel = await prisma.canalIntegracao.update({
    ...
    data: {
      ...
      lastFailureCode: "INSTAGRAM_SYNTHETIC_FAILURE",
    },
  });
  response = await action("pause", target.empresaId, lifecycleBody(channel.updatedAt), operator.token);
  assert.equal(response.status, 200);
  ...
  assert.equal(pausedAfterFailure.lastFailureCode, "INSTAGRAM_SYNTHETIC_FAILURE");
});

test("lifecycle Instagram reverte canal e capabilities se auditoria falhar", async () => {
  const operator = await register(
    "Operadora Rollback Instagram",
    "Operadora",
    uniqueEmail("operator-rollback"),
  );
  const target = await register(
    "Tenant Rollback Instagram",
    "Admin Alvo",
  ...
  } = require("../src/integrations/instagramInboundLifecycle");
  const lifecycle = createInstagramInboundLifecycleService({ prisma, logger: { info() {} } });
  ...
  const loggerFailure = createInstagramInboundLifecycleService({
    prisma,
...
test("lifecycle Instagram serializa ativacao concorrente sem duplicar auditoria", async () => {
  const operator = await register(
    "Operadora Concorrencia Instagram",
    "Operadora",
    uniqueEmail("operator-concurrency"),
  );
  const target = await register(
    "Tenant Concorrencia Instagram",
    "Admin Alvo",
  ...
  assert.equal(results.every((result) => (
    result.status === 200 || (
      result.status === 409 && result.body.codigo === "INSTAGRAM_CHANNEL_CONFLICT"
    )
  )), true);
...
async function register(empresaNome, adminNome, email) {
  const senha = "SenhaInstagramLifecycle123";
...
async function createRealChannel(empresaId, overrides = {}) {
  return prisma.canalIntegracao.create({
    data: {
      empresaId,
      tipo: "INSTAGRAM_META",
      nome: "Instagram inbound lifecycle",
      chaveInterna: "instagram-meta-inbound-real",
      publicId: `instagram-lifecycle-${empresaId}-${suffix}`,
      status: "INATIVO",
      modoTeste: false,
      ativo: false,
      providerEnvironment: process.env.INSTAGRAM_PROVIDER_ENVIRONMENT,
      metaAppId: process.env.INSTAGRAM_META_APP_ID,
      instagramBusinessAccountId: `IG_LIFECYCLE_${empresaId}_${suffix}`,
      instagramUsernameMasked: "@insta***",
      ...overrides,
    },
  });
}
...
async function createTestChannel(empresaId) {
  return prisma.canalIntegracao.create({
    data: {
      empresaId,
      tipo: "INSTAGRAM_META",
      nome: "Instagram test lifecycle",
      chaveInterna: "instagram-meta-test-lifecycle",
      publicId: `instagram-lifecycle-test-${empresaId}-${suffix}`,
      status: "MODO_TESTE",
      modoTeste: true,
      ativo: true,
      instagramBusinessAccountId: `IG_TEST_${empresaId}_${suffix}`,
    },
  });
}
...
function lifecycleBody(updatedAt) {
  return {
    ...
    reason: "Operacao controlada do lifecycle Instagram",
  };
}
...
async function action(name, tenantId, body, token, headers) {
  return request(
    "POST",
    `/platform/tenants/${tenantId}/integrations/instagram/inbound/${name}`,
    body,
    token,
    headers,
  );
